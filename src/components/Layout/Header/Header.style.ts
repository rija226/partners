...
// Exactly the original flex + space-between layout (desktop was never broken until this got
// swapped for a grid to make room for a 4th "MobileMenu" flex child — don't repeat that
// mistake: MobileMenu's toggle is positioned independently via its own CSS instead, see
// MobileMenu.style.ts, so it never has to be a sibling in this row).
